...
1. Supports embedded covers and LRC lyrics. Current wrapper versions provide the required `media-user-token` automatically.
...
AAC-LC, music videos, and lyrics require a valid `media-user-token`. The downloader gets it from the wrapper account endpoint by default; only older wrapper versions require manual configuration.
...
1. Make sure the decryption program [wrapper](https://github.com/WorldObservationLog/wrapper) is running with its account endpoint available (default `127.0.0.1:30020`).
...
Current wrapper versions provide the required `media-user-token` through `get-account-port`, so manual configuration is normally unnecessary.

When using an older wrapper without the account endpoint, retrieve the `media-user-token` cookie from [Apple Music](https://music.apple.com) Developer Tools and set it in `config.yaml` as a fallback.
